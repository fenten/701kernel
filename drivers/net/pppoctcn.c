...
struct sk_buff *pppoctcn_accm(const unsigned char *buf,
...
static int pppoctcn_xmit(struct ppp_channel *chan, struct sk_buff *skb)
{
	...
	*buf++ = PPP_FLAG;
	...
	/*FIXME e13875 we should only put in address/control bytes
	 *if COMP_AC is not negotiated, but how ?*/
	...
	*buf++ = PPP_FLAG;
	...
	outbuf[2] = 0;
	outbuf[3] = 0;
	outbuf[4] = (opt->streamid  >> 24) & 0xFF;
	outbuf[5] = (opt->streamid  >> 16) & 0xFF;
	outbuf[6] = (opt->streamid  >> 8) & 0xFF;
	outbuf[7] = (opt->streamid) & 0xFF;
	...
	int addrlen, int flags)
{
	...
	po->proto.ctcn.streamid = addr->streamid;
...
static struct proto_ops pppoctcn_proto_ops = {
	.family = PF_PPPOX,
	.owner = THIS_MODULE,
